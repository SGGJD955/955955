...
{
    public class ThreadTaskLogService
    {
        /// <summary>
        /// 获取列表
        /// </summary>
        ...
        public DataTable GetList(string where)
        {
            try
            {
                DataTable dt = AccessHelper.ExecuteDataSet(AccessHelper.conn, "select * from ThreadLog where " + where).Tables[0];
        ...
        public int Remove(string GUID)
        {
            try
            {
                int mm = AccessHelper.ExecuteNonQuery(AccessHelper.conn, "delete from ThreadLog where [GUID]=@GUID", new OleDbParameter("@GUID", GUID));
