...
namespace Angels.Application.TicketEntity.Common.BaiduMapTileDownload
{
    public class CoordTransferHelper
    {
        ...
        //以下是根据百度地图JavaScript API破解得到（百度坐标<->墨卡托坐标）转换算法相关参数
        //经纬度参数
        ...
        //墨卡托坐标转百度
        public static LatLngPoint Mercator2LatLng(PointF p)
        {
            double[] arr = null;
        ...
        //高德瓦片（tileX， tileY）的像素坐标（pixelX, pixelY）转经纬度坐标（lng, lat）
        public static LatLngPoint PixelXY2LatLng(Point TileXY, Point PixelXY, int level)
        {
            double lng = (TileXY.X + PixelXY.X / 256.0) / Math.Pow(2, level) * 360.0 - 180.0;
            double lat = Math.Atan(Math.Sinh(Math.PI - 2 * Math.PI * ((TileXY.Y + PixelXY.Y / 256.0) / Math.Pow(2, level)))) * 180.0 / Math.PI;
